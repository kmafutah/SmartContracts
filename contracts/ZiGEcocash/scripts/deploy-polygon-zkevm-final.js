...
async function main() {
    ...
    console.log("🚀 Deploying ZiG Ecosystem to Polygon zkEVM (Final)...");
    ...
    const northAfricaBalance = await regionalStablecoins.getBalance(1, deployer.address);
    const westAfricaBalance = await regionalStablecoins.getBalance(2, deployer.address);
    console.log(`  North Africa Balance: ${ethers.formatEther(northAfricaBalance)} tokens`);
    console.log(`  West Africa Balance: ${ethers.formatEther(westAfricaBalance)} tokens`);
    ...
    // Test remittance with smaller amount
    console.log("\n🧪 Testing remittance...");
    const remittanceAmount = ethers.parseEther("25"); // Use smaller amount
    const remittanceTx = await regionalStablecoins.remittance(
        2, // WEST_AFRICA
        deployer.address, // to self for testing
        remittanceAmount, // 25 tokens
    ...
    const fs = require('fs');
    fs.writeFileSync(
        'deployment-polygon-zkevm-final.json',
        JSON.stringify(deploymentInfo, null, 2)
    );
    console.log("\n💾 Deployment info saved to deployment-polygon-zkevm-final.json");
