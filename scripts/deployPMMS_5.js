...
async function main() {
  ...
  for (const contractName of strategyContracts) {
    ...
    const strategy = await Strategy.deploy(); // Assumes no constructor params
    await strategy.waitForDeployment();
    const strategyAddress = await strategy.getAddress();

    // Derive strategy name (remove "Strategy" prefix)
    const strategyName = contractName.replace(/^Strategy/, "");
    await registry.registerStrategy(strategyName, strategyAddress);
  ...
  if (!["hardhat", "localhost"].includes(network)) {
    ...
    // Verify non-upgradeable strategies
    for (const [contractName, address] of Object.entries(strategyInfo)) {
      try {
        ...
        console.log(`🔍 Verified strategy: ${contractName} at ${address}`);
      } catch (e) {
